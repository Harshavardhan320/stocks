...
public class HoldingsService {
	...
	public Holdings SellStocks(QuantityBuyorSell quantityBuyorSell)throws StockException, AmountException, CalandarException{
		try {
			...
			long monthid = this.yearAndMonthService.getYearAndMonthId();
			if(monthid != holdings.getYearMonth().getId()) {
				throw new StockException("You can't sell extra quanity to the last month data");
			}
	...
	public Holdings updateCrrPrice(UpDateCrrPrice upDateCrrPrice) throws StockException, CalandarException {
		...
		{	
			 ...
			 * 
			 */
			Optional<Holdings> lastMonthData =  this.holdingesRepository.findByIdStockNameAndStatusAndMonth(id, stockName, holdingsmonthYearId);
	 ...
	 * 	-->7.2 checking editing month.
	 * 	-->7.3 checking stock is present in holdings.
	...
	public int editStockPriceduration(UpDateCrrPrice upDateCrrPrice, String duration, double squarOffValue) throws CalandarException, StockException {
		...
		try {
			...
			long id = upDateCrrPrice.getId();
			String stockName = upDateCrrPrice.getStockName();
			BigDecimal currentPrice = new BigDecimal(String.valueOf(upDateCrrPrice.getCurrentPrice()));
			long monthId = upDateCrrPrice.getMonthYearId();
			
			String Stockduration = duration;
			...
			if(monthId != yearMonthId) {
				throw new StockException("You can't edit the stock");
			}
			
			/*
			 * 	-->7.3 checking stock is present in holdings
			 */
			Optional<Holdings> holdingsdata = this.holdingesRepository.findByIdandName(id, stockName);
			if(holdingsdata.isEmpty()) {
				throw new StockException("Stock Not found in holdings");
			}
			...
			Holdings holdings = holdingsdata.get();
			
			if(holdings.getYearMonth().getId() != yearMonthId) {
				throw new StockException("You can't edit last month data or futute months data");
			}
			...
			int holdings1 = this.holdingesRepository.changingDuration(Stockduration, stockName);
